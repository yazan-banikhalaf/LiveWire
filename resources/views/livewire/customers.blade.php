    ...
    @if($showForm)
        <form wire:submit.prevent="create" class="mb-4 p-4 rounded shadow-lg">
            <div class="mb-4">
                ...
                <x-text-input wire:model="name" id="name" class="block mt-1 w-full" type="text" name="name" : placeholder="Name Customer..." />
                @error('name') <span class="text-red-500 text-xs">{{$message}}</span> @enderror
            </div>

            <div class="mb-4">
                <label class="block  text-sm font-bold mb-2" for="phone">Phone</label>
                <x-text-input wire:model="phone" id="phone" class="block mt-1 w-full" type="text" name="phone" : placeholder="phone Customer..." />
                @error('phone') <span class="text-red-500 text-xs">{{$message}}</span> @enderror
            </div>

            <div class="mb-4">
                <label class="block  text-sm font-bold mb-2" for="name">Email</label>
                <x-text-input wire:model="email" id="email" class="block mt-1 w-full" type="email" name="email" : placeholder="email Customer..." />
            ...
            @foreach($customers as $customer)
                <tr class="bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-600">
                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                        <div class="text-gray-900 dark:text-gray-300">{{$customer->id}}</div>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                        <div class="text-gray-900 dark:text-gray-300">{{$customer->name}}</div>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                        <div class="text-gray-900 dark:text-gray-300">{{$customer->email}}</div>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                        <div class="text-gray-900 dark:text-gray-300">{{$customer->phone}}</div>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 text-sm">
                        <div class="flex">
                            <button wire:click="edit({{ $customer->id }})" class="text-blue-500 dark:text-blue-300 hover:text-blue-700 dark:hover:text-blue-500 mr-2">
                                Edit
                            </button>
                            <button wire:click="confirmDeletion({{ $customer->id }})" class="text-red-500 dark:text-red-300 hover:text-red-700 dark:hover:text-red-500">
                                Delete
                            </button>
                        </div>
                    </td>
            ...
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-orange-200">
                <div class="mt-3 text-center">
                    ...
                    <div class="items-center px-4 py-3">
                        <button wire:click="destroy({{ $deletingCustomerId }})" class="px-4 py-2 bg-red-500 text-white text-base font-medium rounded-md w-5/12 shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            Delete
                        </button>
